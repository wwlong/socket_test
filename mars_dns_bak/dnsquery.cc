...
int socket_gethostbyname(const char* _host, socket_ipinfo_t* _ipinfo, int _timeout /*ms*/, const char* _dnsserver) {
   ...
   // std::vector<std::string>::iterator iter = dns_servers.begin();//设置dns服务器ip容器
    ...
    printf("recvPacketLen:%d\n",recvPacketLen);
        ...
        printf("\r\n");
        printf("recv_src.sin6_port : %d\r\n", recv_src.sin6_port);
        printf("recv_src.sin6_addr.s6_addr : ");
        for(int i = 0;i < 16;i ++) {
            printf("%02x", recv_src.sin6_addr.s6_addr[i]);
            if((i+1)%2 == 0) {
                printf(":");
            }
        }
        printf("\r\n");
        printf("recv_src.sin6_scope_id : %d\r\n", recv_src.sin6_scope_id);
